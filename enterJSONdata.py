...
def enterFlightLog(logs):
    ...
    while True:
        ...
        print("|-------------------------------------------------------------|")
        print()
        flightNumberList = sectionTable(
            "Flight Number", """       Existing Flight Numbers: """, 'flightNumber', logs)
...
####################################################################################
    ...
    system('cls')
    aircraftIDList = sectionTable(
        "Aircraft ID", """        Choose the Aircraft ID """, 'aircraft.id', logs)
...
####################################################################################
    system('cls')
    aircraftMakeList = sectionTable(
        "Aircraft Make", """        Choose the Aircraft Make """, 'aircraft.make', logs)
...
####################################################################################
    system('cls')
    aircraftModelList = sectionTable(
        "Aircraft Model", """        Choose the Aircraft Model """, 'aircraft.model', logs)
...
####################################################################################
    ...
    while True:
        ...
        print("Entered Passengers: " + Fore.GREEN +
              str(passengers) + Style.RESET_ALL)
    ...
    if len(optionList) <= 5:
        for i in range(len(optionList)):
            line.append('[' + str(i) + '] - ' + Fore.GREEN +
                        str(optionList[i]) + Style.RESET_ALL)
            tbl.add_row(line)
            line = []
    else:
        for i in range(len(optionList)):
    ...
    if searchKey != 'passengers':
        if '.' in searchKey:
            for i in range(len(jsonData)):
                # print(jsonData[i]['aircraft']['model'])
                values.add(jsonData[i]['aircraft'][searchKey.split('.')[1]])
        else:
